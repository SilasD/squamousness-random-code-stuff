do dfhack.printerr("attempt to run a profiled script!"); return; end;  local profile = '2'; myprofiler = reqscript("myprofiler"); myprofiler.stop(); if profile then _G.__ruins_state = nil; myprofiler.start(); end --[====[ ruins.lua
...
local function original_tile_for(tt)                                                                                    --   75679  0.677s
    local attrs = df.tiletype.attrs[tt]                                                                                 --   12561
    if attrs then                                                                                                       --    4087
        local m = attrs.material                                                                                        --    4461
        if m == df.tiletype_material.GRASS_DARK or m == df.tiletype_material.GRASS_LIGHT then                           --   36897
            -- SWD: Q: what about GRASS_DRY and GRASS_DEAD ?
            return SOIL_FLOOR_TT or tt                                                                                  --   17139
        end
    end
    return tt                                                                                                           --     534
end
...
-- Suffixes with no matching tiletype (single-connection stubs) become pillars.
local function wall_tt(suffix)                                                                                          --    1152  0.016s
    return constructed_wall_by_suffix[suffix]                                                                           --    1152
...
local function dlog(msg, ...) if S.debug then log("DIAG: " .. msg, ...) end end                                         --       2  0.006s (self 0.006s) [child 0.000s]
...
---@param z integer
---@return key_xyz
local function key_xyz(x, y, z)                                                                                         --   59595  0.644s
    ---@diagnostic disable-next-line: return-type-mismatch
    return ("%d,%d,%d"):format(x, y, z)                                                                                 --   59595
end
...
local function build_road_set( --[[ block_map SWD disabled ]] )                                                         --    4932  0.042s (self 0.027s) [child 0.015s]
    ...
    local function seed(b)                                                                                              --     199  0.005s
        --dlog("road seed %d,%d,%d", b.x1, b.y1, b.z)
        for rx = b.x1, b.x2 do
            for ry = b.y1, b.y2 do                                                                                      --      50
                table.insert(queue, rx); table.insert(queue, ry); table.insert(queue, b.z)                              --     137
            end
        end
    end
    ...
    while i <= #queue do                                                                                                --      44
        ...
        local x, y, z = queue[i], queue[i+1], queue[i+2]; i = i + 3                                                     --     228
        local key = key_xyz(x, y, z)                                                                                    --     365
        if tested[key] then goto continue end                                                                           --     446
        tested[key] = true                                                                                              --     108
        local tt = dfhack.maps.getTileType(x, y, z) or OPEN_SPACE_TT                                                    --     218
        local shape = df.tiletype.attrs[tt].shape                                                                       --      70

        -- only floors and ramps can be roads, but see below.
        if shape == DF_tiletype_shape_FLOOR or shape == DF_tiletype_shape_RAMP then
            local des, occ = dfhack.maps.getTileFlags(x, y, z)                                                          --     173
            if occ.no_grow and occ.building == 0 then                                                                   --     208
                road_set[key] = true                                                                                    --      27
                table.insert(road_tiles, x); table.insert(road_tiles, y); table.insert(road_tiles, z)                   --     255
                -- and queue up the orthoganal tiles to be tested.
                for d = 1, 4 do                                                                                         --      59
                    local nx, ny = x + dx[d], y + dy[d]                                                                 --     424
                    local k = key_xyz(nx, ny, z)                                                                        --     327
                    if not tested[k] then                                                                               --     841
                        table.insert(queue, nx); table.insert(queue, ny); table.insert(queue, z)                        --     749
                    end
                end
            end
        end
        ...
        if shape == DF_tiletype_shape_RAMP and road_set[key] then                                                       --      27
            local zup = z + 1
            local keyup = key_xyz(x, y, zup)
            local ttup = dfhack.maps.getTileType(x, y, zup) or OPEN_SPACE_TT                                            --      28
            local shapeup = df.tiletype.attrs[ttup].shape
            if shapeup == DF_tiletype_shape_RAMP_TOP then
                tested[keyup] = true
                road_set[keyup] = true
                table.insert(road_tiles, x); table.insert(road_tiles, y); table.insert(road_tiles, zup)                 --      30
                -- queue up the orthoganal tiles to be tested.
                for d = 1, 4 do
                    local nx, ny = x + dx[d], y + dy[d]
                    local k = key_xyz(nx, ny, zup)
                    if not tested[k] then                                                                               --      93
                        table.insert(queue, nx); table.insert(queue, ny); table.insert(queue, zup)                      --      75
                    end
                end
            end
        end
...
local function build_existing_set()                                                                                     --       1  0.001s (self 0.001s) [child 0.000s]
    local existing = {}
...
local function hash_percent(x, y, z)                                                                                    --  107639  0.643s
    local h = 1234567                                                                                                   --   19917
    h = (h * 1103515245 + x + 12345) & 0x7FFFFFFF                                                                       --    6047
    h = (h * 1103515245 + y + 12345) & 0x7FFFFFFF                                                                       --   24613
    h = (h * 1103515245 + z + 12345) & 0x7FFFFFFF                                                                       --   36201
    return h % 100                                                                                                      --   20861
end

-- Returns the maximum wall height.
local function get_biome_max_height(wx, wy, wz)                                                                         --   65499  0.125s
    local rx, ry = dfhack.maps.getTileBiomeRgn(wx, wy, wz)                                                              --   16108
    local ck     = rx .. "," .. ry                                                                                      --   15905
    if S.biome_height_cache[ck] then return S.biome_height_cache[ck] end                                                --   33485
...
local function get_biome_mat(wx, wy, wz)                                                                                --  112740  0.317s
    local block = dfhack.maps.getTileBlock(wx, wy, wz)                                                                  --   12711
    if block and block.designation[wx % 16][wy % 16].subterranean then                                                  --   43259
        return SUPERSTRUCTURE_MAT_TYPE, SUPERSTRUCTURE_MAT_INDEX                                                        --   26980
    end

    local rx, ry = dfhack.maps.getTileBiomeRgn(wx, wy, wz)                                                              --    8471
    local ck     = rx .. "," .. ry                                                                                      --    6072
    if S.biome_mat_cache[ck] then                                                                                       --    4854
        local m = S.biome_mat_cache[ck]                                                                                 --    3740
        return m[1], m[2]                                                                                               --    6651
    end
    local btype = dfhack.maps.getBiomeType(rx, ry)
    local mt, mi
...
local function is_floor_only_biome(wx, wy, wz)                                                                          --   16072  0.079s
    local rx, ry = dfhack.maps.getTileBiomeRgn(wx, wy, wz)                                                              --     860
    local ck     = rx .. "," .. ry                                                                                      --    1115
    if S.biome_floor_cache[ck] ~= nil then return S.biome_floor_cache[ck] end                                           --   14097
    local btype  = dfhack.maps.getBiomeType(rx, ry)
...
local function get_tower_height(wx, wy, wz, max_h)                                                                      --    9310  0.055s (self 0.026s) [child 0.029s]
    local roll = hash_percent(wx + 31337, wy + 271, wz)                                                                 --     929
    return 1 + math.floor(roll * max_h / 100)                                                                           --    8381
end

-- True if the CONCRETE tile at (wx,wy,wz) will have an upper wall at z+tier.
local function will_have_tier(wx, wy, wz, wall_set, tier)                                                               --    3770  0.225s (self 0.094s) [child 0.131s]
    if not wall_set[key_xyz(wx, wy, wz)] then return false end                                                          --    3039
    local max_h = get_biome_max_height(wx, wy, wz)                                                                      --     177
    if tier > max_h - 1 then return false end                                                                           --      50
    return get_tower_height(wx, wy, wz, max_h) > tier                                                                   --     504
end

-- Collect positions of all CONCRETE grass tiles from a block list.
local function collect_wall_positions(block_list)                                                                       --   31891  0.689s (self 0.634s) [child 0.055s]
    local wall_set = {}
    for _, block in ipairs(block_list) do                                                                               --     240
        for _, ev in ipairs(block.block_events) do                                                                      --    1500
            if getmetatable(ev) == "block_square_event_grassst"                                                         --    1225
                ---@cast ev df.block_square_event_grassst
                    and plant_role_cache[ev.plant_index] == "wall" then                                                 --     155
                for lx = 0, 15 do                                                                                       --     356
                    for ly = 0, 15 do                                                                                   --    6037
                        if ev.amount[lx][ly] > 0 then                                                                   --   17414
                            wall_set[key_xyz(                                                                           --     530
                                block.map_pos.x + lx,                                                                   --     752
                                block.map_pos.y + ly,                                                                   --     811
                                block.map_pos.z)] = true                                                                --    2871
                        end
                    end
                end
            end
...
local function is_constructed_wall_tt(tt)                                                                               --    7318  0.134s
    if not tt then return false end                                                                                     --     522
    local attrs = df.tiletype.attrs[tt]                                                                                 --    1845
    if not attrs then return false end                                                                                  --    1132
    if attrs.material ~= DF_TILETYPE_MATERIAL_CONSTRUCTION then return false end                                        --    2475
    local s = attrs.shape                                                                                               --     258
    return s == DF_TILETYPE_SHAPE_WALL or s == DF_TILETYPE_SHAPE_PILLAR                                                 --    1086
end

-- True if (wx,wy) is a wall neighbour at z-level zt:
-- either a CONCRETE grass tile or an already-converted ConstructedWall.
local function is_wall_neighbour(wx, wy, wz, wall_set)                                                                  --   12314  0.142s (self 0.099s) [child 0.043s]
    if wall_set[key_xyz(wx, wy, wz)] then return true end                                                               --    7708
    return is_constructed_wall_tt(dfhack.maps.getTileType(wx, wy, wz))                                                  --    4606
end

local function wall_suffix(wx, wy, wz, wall_set)                                                                        --   20660  0.325s (self 0.183s) [anon child 0.106s] [child 0.142s]
    local s = ""                                                                                                        --      30
    if is_wall_neighbour(wx - 1, wy,     wz, wall_set) then s = s .. "L" end                                            --   14482
    if is_wall_neighbour(wx + 1, wy,     wz, wall_set) then s = s .. "R" end                                            --    3191
    if is_wall_neighbour(wx,     wy - 1, wz, wall_set) then s = s .. "U" end                                            --     524
    if is_wall_neighbour(wx,     wy + 1, wz, wall_set) then s = s .. "D" end                                            --    2193
    return s                                                                                                            --     240
end
...
local function upper_wall_suffix(wx, wy, wz, wall_set, tier)                                                            --    1681  0.398s (self 0.047s) [anon child 0.028s] [child 0.350s]
    local zt = wz + tier                                                                                                --     348
    ...
    local s = ""                                                                                                        --      71
    if has_upper(wx - 1, wy    ) then s = s .. "L" end                                                                  --      97
    if has_upper(wx + 1, wy    ) then s = s .. "R" end                                                                  --     205
    if has_upper(wx,     wy - 1) then s = s .. "U" end                                                                  --     247
    if has_upper(wx,     wy + 1) then s = s .. "D" end                                                                  --     600
    return s                                                                                                            --      90
end
...
local function zero_grass_at(block, lx, ly)                                                                             --    4201  0.022s
    for _, ev in ipairs(block.block_events) do                                                                          --    3466
        if getmetatable(ev) == "block_square_event_grassst" then                                                        --     534
            ---@cast ev df.block_square_event_grassst
            ev.amount[lx][ly] = 0                                                                                       --     130
        end
    end
end                                                                                                                     --      71
...
local function insert_upper_wall(wx, wy, wz, existing, wall_set, tier, mat_type, mat_index, override_tt)                --  176464  0.914s (self 0.447s) [child 0.466s]
    local k = key_xyz(wx, wy, wz)                                                                                       --    1742
    if existing[k] then return false end                                                                                --    2744
    local block = dfhack.maps.getTileBlock(wx, wy, wz)                                                                  --    9507
    if not block then return false end                                                                                  --    1810
    local lx, ly = wx % 16, wy % 16                                                                                     --    4604
    local attrs = df.tiletype.attrs[block.tiletype[lx][ly]]                                                             --   20442
    if not attrs or attrs.material ~= df.tiletype_material.AIR then return false end                                    --   29203

    local c = df.construction:new()                                                                                     --   12399
    c.pos.x     = wx                                                                                                    --    6997
    c.pos.y     = wy                                                                                                    --    7330
    c.pos.z     = wz                                                                                                    --    7626
    c.mat_type  = mat_type                                                                                              --    4163
    c.mat_index = mat_index                                                                                             --    4227
    c.item_type = df.item_type.BLOCKS                                                                                   --   17276
    c.original_tile = original_tile_for(block.tiletype[lx][ly])                                                         --   22877
    c.flags.no_build_item = true                                                                                        --     189

    if not dfhack.constructions.insert(c) then return false end                                                         --     907

    existing[k] = true                                                                                                  --     207
    if override_tt then                                                                                                 --     223
        block.tiletype[lx][ly] = override_tt
    else
        local suffix = upper_wall_suffix(wx, wy, wz - tier, wall_set, tier)                                             --    1992
        local tt = wall_tt(suffix)                                                                                      --     689
        if tt then block.tiletype[lx][ly] = tt end                                                                      --    5926
    end
    zero_grass_at(block, lx, ly)                                                                                        --   13176
    return true                                                                                                         --     200
end
...
local function insert_cap_floor(wx, wy, wz, existing, mat_type, mat_index)                                              --   21012  0.247s (self 0.182s) [child 0.065s]
    local k = key_xyz(wx, wy, wz)                                                                                       --    3125
    if existing[k] then return end                                                                                      --     315
    local block = dfhack.maps.getTileBlock(wx, wy, wz)                                                                  --    1207
    if not block then return end                                                                                        --     281
    local lx, ly = wx % 16, wy % 16                                                                                     --     611
    local attrs  = df.tiletype.attrs[block.tiletype[lx][ly]]                                                            --    2736
    if not attrs or attrs.material ~= df.tiletype_material.AIR then return end                                          --    3839
    local c = df.construction:new()                                                                                     --    1138
    c.pos.x     = wx                                                                                                    --     610
    c.pos.y     = wy                                                                                                    --     648
    c.pos.z     = wz                                                                                                    --     732
    c.mat_type  = mat_type                                                                                              --     365
    c.mat_index = mat_index                                                                                             --     383
    c.item_type = df.item_type.BLOCKS                                                                                   --    1513
    c.original_tile = original_tile_for(block.tiletype[lx][ly])                                                         --    2261
    c.flags.no_build_item = true                                                                                        --     210
    if dfhack.constructions.insert(c) then                                                                              --     442
        existing[k] = true                                                                                              --      48
        if CONSTRUCTED_FLOOR_TT then                                                                                    --     107
            block.tiletype[lx][ly] = CONSTRUCTED_FLOOR_TT                                                               --     298
        end
    end
end                                                                                                                     --     143
...
local function do_wall_span(wx, wy, wz, dx, dy, existing, mat_type, mat_index)                                          --   14266  0.919s (self 0.435s) [child 0.484s]
    local placed = {}  -- { {nx, ny, is_fort}, ... }                                                                    --     712

    for step = 1, MAX_SPAN do                                                                                           --     847
        local nx, ny   = wx + dx * step, wy + dy * step                                                                 --     271
        local block    = dfhack.maps.getTileBlock(nx, ny, wz)                                                           --     975
        if not block then break end                                                                                     --     108
        local lx, ly   = nx % 16, ny % 16                                                                               --     332
        local attrs    = df.tiletype.attrs[block.tiletype[lx][ly]]                                                      --     793
        if not attrs or attrs.material ~= df.tiletype_material.AIR then break end                                       --     669
        if hash_percent(nx + 313, ny + 17, wz) < SPAN_WALL_GAP_PROB then break end                                      --     484
        local k = key_xyz(nx, ny, wz)                                                                                   --     209
        if not existing[k] then                                                                                         --     129
            local is_fort = CONSTRUCTED_FORTIFICATION_TT ~= nil                                                         --     243
                            and mat_type == SUPERSTRUCTURE_MAT_TYPE                                                     --     119
                            and mat_index == SUPERSTRUCTURE_MAT_INDEX                                                   --     313
                            and hash_percent(nx, ny + 500, wz) < SPAN_WALL_FORT_PROB                                    --      74
            local c = df.construction:new()                                                                             --     281
            c.pos.x = nx; c.pos.y = ny; c.pos.z = wz                                                                    --     187
            c.mat_type = mat_type; c.mat_index = mat_index                                                              --     105
            c.item_type = df.item_type.BLOCKS                                                                           --     424
            c.original_tile = original_tile_for(block.tiletype[lx][ly])                                                 --     353
            c.flags.no_build_item = true
            if dfhack.constructions.insert(c) then                                                                      --      58
                existing[k] = true
                block.tiletype[lx][ly] = CONSTRUCTED_WALL_TT  -- placeholder                                            --     176
                placed[#placed + 1] = { nx, ny, is_fort }                                                               --     538
            end
        end
    end

    -- Re-suffix every placed tile now that all span neighbours are in the map
    for _, p in ipairs(placed) do                                                                                       --     497
        local nx, ny, is_fort = p[1], p[2], p[3]                                                                        --     481
        local blk = dfhack.maps.getTileBlock(nx, ny, wz)                                                                --     950
        if blk then                                                                                                     --     150
            if is_fort then                                                                                             --     102
                blk.tiletype[nx % 16][ny % 16] = CONSTRUCTED_FORTIFICATION_TT
            else
                blk.tiletype[nx % 16][ny % 16] = wall_tt(wall_suffix(nx, ny, wz, {}))                                   --    1586
            end
        end
    end
	
    for _, p in ipairs(placed) do                                                                                       --     712
        insert_cap_floor(p[1], p[2], wz + 1, existing, mat_type, mat_index)                                             --    1295
    end
end                                                                                                                     --      67

-- Floor slabs with ragged side edges.
-- Support propagates horizontally from the tower wall through the connected floor chain
local function do_floor_span(wx, wy, wz, dx, dy, existing, mat_type, mat_index)                                         --   25145  1.773s (self 0.954s) [child 0.819s]
    local pdx, pdy = -dy, dx                                                                                            --    1194

    local function try_floor(nx, ny)                                                                                    --       4  0.000s (self 0.000s) [child 0.000s]--       6  0.000s (self 0.000s) [child 0.000s]--       7  0.000s (self 0.000s) [child 0.000s]--       3  0.000s (self 0.000s) [child 0.000s]--       3  0.000s (self 0.000s) [child 0.000s]--       5  0.000s (self 0.000s) [child 0.000s]--       3  0.000s (self 0.000s) [child 0.000s]--       4  0.000s (self 0.000s) [child 0.000s]--       1  0.000s (self 0.000s) [child 0.000s]--       3  0.000s (self 0.000s) [child 0.000s]--       3  0.000s (self 0.000s) [child 0.000s]--       6  0.000s (self 0.000s) [child 0.000s]--       4  0.000s (self 0.000s) [child 0.000s]--       7  0.000s (self 0.000s) [child 0.000s]--       6  0.000s--       7  0.000s (self 0.000s) [child 0.000s]--       4  0.000s--       2  0.000s (self 0.000s) [child 0.000s]--       7  0.000s (self 0.000s) [child 0.000s]--       4  0.000s (self 0.000s) [child 0.000s]--       5  0.000s (self 0.000s) [child 0.000s]--       3  0.000s (self 0.000s) [child 0.000s]--       4  0.000s (self 0.000s) [child 0.000s]--       6  0.000s (self 0.000s) [child 0.000s]--       4  0.000s (self 0.000s) [child 0.000s]--       2  0.000s (self 0.000s) [child 0.000s]--       3  0.000s (self 0.000s) [child 0.000s]--       3  0.000s (self 0.000s) [child 0.000s]--       4  0.000s (self 0.000s) [child 0.000s]--       4  0.000s (self 0.000s) [child 0.000s]--       3  0.000s (self 0.000s) [child 0.000s]--       4  0.000s (self 0.000s) [child 0.000s]--       5  0.000s (self 0.000s) [child 0.000s]--       2  0.000s (self 0.000s) [child 0.000s]--       5  0.000s--       2  0.000s (self 0.000s) [child 0.000s]--       4  0.000s (self 0.000s) [child 0.000s]--       3  0.000s (self 0.000s) [child 0.000s]--       5  0.000s (self 0.000s) [child 0.000s]--       4  0.000s (self 0.000s) [child 0.000s]--       3  0.000s (self 0.000s) [child 0.000s]--       4  0.000s (self 0.000s) [child 0.000s]--       5  0.000s (self 0.000s) [child 0.000s]--       5  0.000s (self 0.000s) [child 0.000s]--       3  0.000s (self 0.000s) [child 0.000s]--       2  0.000s (self 0.000s) [child 0.000s]--       4  0.000s (self 0.000s) [child 0.000s]--       4  0.000s (self 0.000s) [child 0.000s]--       8  0.000s (self 0.000s) [child 0.000s]--       3  0.000s (self 0.000s) [child 0.000s]--       5  0.000s (self 0.000s) [child 0.000s]--       3  0.000s (self 0.000s) [child 0.000s]
    ...
    for step = 1, MAX_SPAN do                                                                                           --     441
        local nx, ny = wx + dx * step, wy + dy * step                                                                   --     769
        local block  = dfhack.maps.getTileBlock(nx, ny, wz)                                                             --    1809
        if not block then break end                                                                                     --     321
        local lx, ly = nx % 16, ny % 16                                                                                 --     622
        local attrs  = df.tiletype.attrs[block.tiletype[lx][ly]]                                                        --    2151
        if not attrs or attrs.material ~= df.tiletype_material.AIR then break end                                       --    1813
		
        local k = key_xyz(nx, ny, wz)                                                                                   --     874
        if not existing[k] then                                                                                         --     373
            local c = df.construction:new()                                                                             --     917
            c.pos.x = nx; c.pos.y = ny; c.pos.z = wz                                                                    --    1451
            c.mat_type = mat_type; c.mat_index = mat_index                                                              --     470
            c.item_type = df.item_type.BLOCKS                                                                           --     853
            c.original_tile = original_tile_for(block.tiletype[lx][ly])                                                 --    1356
            c.flags.no_build_item = true                                                                                --     453
            if dfhack.constructions.insert(c) then                                                                      --    1365
                existing[k] = true                                                                                      --     149
                if CONSTRUCTED_FLOOR_TT then block.tiletype[lx][ly] = CONSTRUCTED_FLOOR_TT end                          --    1235
            else
                break
            end
        end
		
        if existing[k] then                                                                                             --     419
            for _, side in ipairs({ 1, -1 }) do                                                                         --    1762
                if hash_percent(nx + side * 200, ny + side * 100, wz) < SPAN_SIDE_PROB then                             --    2604
                    if hash_percent(nx + side * 200, ny + side * 300, wz) >= SPAN_SIDE_GAP_PROB then                    --     854
                        try_floor(nx + pdx * side, ny + pdy * side)                                                     --     725
                    end
                end
            end
        end
...
local function do_catwalk_span(wx, wy, wz, dx, dy, existing, mat_type, mat_index)                                       --   22975  1.186s (self 0.885s) [child 0.300s]
    for step = 1, MAX_SPAN do                                                                                           --    4235
        local nx, ny = wx + dx * step, wy + dy * step                                                                   --    1616
        local block  = dfhack.maps.getTileBlock(nx, ny, wz)                                                             --    3467
        if not block then break end                                                                                     --     469
        local lx, ly = nx % 16, ny % 16                                                                                 --    1101
        local attrs  = df.tiletype.attrs[block.tiletype[lx][ly]]                                                        --    1730
        if not attrs or attrs.material ~= df.tiletype_material.AIR then break end                                       --    2479
        local k = key_xyz(nx, ny, wz)                                                                                   --    1471
        if not existing[k] then                                                                                         --    1027
            local c = df.construction:new()                                                                             --    1257
            c.pos.x = nx; c.pos.y = ny; c.pos.z = wz                                                                    --     345
            c.mat_type = mat_type; c.mat_index = mat_index                                                              --     258
            c.item_type = df.item_type.BLOCKS                                                                           --    1130
            c.original_tile = original_tile_for(block.tiletype[lx][ly])                                                 --     892
            c.flags.no_build_item = true                                                                                --     100
            if dfhack.constructions.insert(c) then                                                                      --     261
                existing[k] = true
                if CONSTRUCTED_FLOOR_TT then block.tiletype[lx][ly] = CONSTRUCTED_FLOOR_TT end                          --     987
            else
                break  
            end
        end
...
local function generate_spans(wx, wy, wz_base, actual_h, existing, mat_type, mat_index)                                 --  457555  4.989s (self 0.858s) [child 4.131s]
    local base_block     = dfhack.maps.getTileBlock(wx, wy, wz_base)                                                    --    2245
    local is_underground = base_block
                           and base_block.designation[wx % 16][wy % 16].subterranean                                    --    5968
    for tier = 0, actual_h do                                                                                           --    8738
        local frac    = actual_h > 0 and (tier / actual_h) or 0                                                         --    7621
        local wall_p  = math.floor(50 * (1 - frac) + 5  * frac + 0.5)                                                   --   25952
        local floor_p = math.floor(38 * (1 - math.abs(2 * frac - 1) ^ 1.4) + 0.5)                                       --   68554
        local ctw_p   = math.floor(5  * (1 - frac) + 35 * frac + 0.5)                                                   --   77981
        local wz      = wz_base + tier                                                                                  --   10442
        for dir_idx, dir in ipairs(SPAN_DIRS) do                                                                        --   57932
            local roll = hash_percent(wx * 4 + dir_idx + 77, wy + tier * 31, wz_base)                                   --  132042
            if roll < wall_p then                                                                                       --     829
                if not NO_WALL_SPANS and tier < actual_h and not is_underground then                                    --    2861
                    do_wall_span(wx, wy, wz, dir[1], dir[2], existing, mat_type, mat_index)                             --    4781
                end                                                                                                     --      69
            elseif roll < wall_p + floor_p then                                                                         --    3345
                if not NO_FLOOR_SPANS then                                                                              --     832
                    do_floor_span(wx, wy, wz, dir[1], dir[2], existing, mat_type, mat_index)                            --    7529
                end                                                                                                     --      93
            elseif roll < wall_p + floor_p + ctw_p then                                                                 --   11008
                if not NO_CATWALK_SPANS then                                                                            --    3420
                    do_catwalk_span(wx, wy, wz, dir[1], dir[2], existing, mat_type, mat_index)                          --   24657
                end
            end
        end
    end
...
local function scan_block(block, existing, wall_set, in_site, road_set)                                                 --  179679  9.854s (self 2.803s) [child 7.051s]
    local floors, walls, uppers = 0, 0, 0                                                                               --     392

    for _, ev in ipairs(block.block_events) do                                                                          --    1131
        if getmetatable(ev) == "block_square_event_grassst" then                                                        --    1090
            ---@cast ev df.block_square_event_grassst
            local role = plant_role_cache[ev.plant_index]                                                               --     113
            if role then                                                                                                --      52
                for lx = 0, 15 do                                                                                       --     764
                    for ly = 0, 15 do                                                                                   --   13057
                        if ev.amount[lx][ly] > 0 then                                                                   --   44038
                            local wx = block.map_pos.x + lx                                                             --     956
                            local wy = block.map_pos.y + ly                                                             --    2144
                            local wz = block.map_pos.z                                                                  --     754
                            local k  = key_xyz(wx, wy, wz)                                                              --    2525
                            -- In NPC settlements, exclude tiles inside/covered by buildings.
                            -- Tiles adjacent to a door are allowed but forced to floors.
                            local in_site_excluded    = false                                                           --    5006
                            local in_site_force_floor = false                                                           --     571
                            if in_site and role == "wall" then                                                          --     415
                            ...
                            if not in_site_excluded then                                                                --     344
                            if road_set and (road_set[k] or road_set[key_xyz(wx, wy, wz - 1)]) then                     --    8099
                                -- Road tile prevents ruin spawning
                                ev.amount[lx][ly] = 0
                            elseif block.occupancy[lx][ly].building == 0                                                --    2493
                                and not block.occupancy[lx][ly].no_grow then                                            --    2595
                                if existing[k] then                                                                     --    1775
								
                                    ev.amount[lx][ly] = 0
                                else
                                local bmt, bmi = get_biome_mat(wx, wy, wz)                                              --    2537
                                local c = df.construction:new()                                                         --   13973
                                c.pos.x     = wx                                                                        --     688
                                c.pos.y     = wy                                                                        --     191
                                c.pos.z     = wz                                                                        --     276
                                c.mat_type  = bmt                                                                       --     189
                                c.mat_index = bmi                                                                       --     139
                                c.item_type = df.item_type.BLOCKS                                                       --    5334
                                c.original_tile = original_tile_for(block.tiletype[lx][ly])                             --    8495
                                c.flags.no_build_item = true                                                            --      55
                                if dfhack.constructions.insert(c) then                                                  --     234
                                    existing[k] = true                                                                  --      37
                                    ev.amount[lx][ly] = 0  -- prevents re-conversion                                    --    6539
                                    local tt_attrs2   = df.tiletype.attrs[block.tiletype[lx][ly]]                       --    8632
                                    local shape_attr2 = tt_attrs2 and df.tiletype_shape.attrs[tt_attrs2.shape]          --    3156
                                    local basic2      = shape_attr2 and shape_attr2.basic_shape                         --    2479
                                    if basic2 == df.tiletype_shape_basic.Ramp then                                      --    2526
                                        if CONSTRUCTED_RAMP_TT then                                                     --      31
                                            block.tiletype[lx][ly] = CONSTRUCTED_RAMP_TT                                --      39
                                        end
                                        floors = floors + 1
                                    elseif role == "floor" or is_floor_only_biome(wx, wy, wz) or in_site_force_floor then--    3273
                                        if CONSTRUCTED_FLOOR_TT then                                                    --     237
                                            block.tiletype[lx][ly] = CONSTRUCTED_FLOOR_TT                               --     384
                                        end
                                        floors = floors + 1
                                    else
                                        -- Height must be known before is_stair so height-1
                                        local max_h    = get_biome_max_height(wx, wy, wz)                               --    2429
                                        local target_h = get_tower_height(wx, wy, wz, max_h)                            --    7458
                                        local is_stair = CONSTRUCTED_STAIR_UD_TT ~= nil                                 --    2467
                                                         and CONSTRUCTED_STAIR_UP_TT ~= nil
                                                         and target_h > 1
                                                         and hash_percent(wx, wy, wz) < STAIR_PROB                      --    6896
                                        local is_fort  = not is_stair
                                                         ...
                                                         and hash_percent(wx, wy + 1, wz) < FORT_PROB                   --    1548
                                        ...
                                        elseif override_tt then
                                            ...
                                            if wall_tt(suffix) then                                                     --     190
                                                block.tiletype[lx][ly] = wall_tt(suffix)                                --     216
                                            end
                                        end
                                        walls = walls + 1
                                        ...
                                        local cap_z    = wz + 1
                                        for tier = 1, target_h - 1 do                                                   --     671
                                            if insert_upper_wall(wx, wy, wz + tier, existing, wall_set, tier, bmt, bmi, override_tt) then--    3531
                                                uppers = uppers + 1                                                     --      38
                                                cap_z  = wz + tier + 1                                                  --     123
                                            else
                                                break
                                        ...
                                        if not is_stair then                                                            --     194
                                            insert_cap_floor(wx, wy, cap_z, existing, bmt, bmi)                         --    1400
                                        ...
                                        local spans_h = is_stair and (cap_z - 1 - wz) or (cap_z - wz)                   --    2460
                                        generate_spans(wx, wy, wz, spans_h, existing, bmt, bmi)                         --    1345
                                    end
                                end
    ...
    return floors, walls, uppers                                                                                        --     755
end

local function get_site_id()                                                                                            --       1  0.003s
    if dfhack.world.getCurrentSite then
        local site = dfhack.world.getCurrentSite()
...
local function convert_ruins(force)                                                                                     --    3585  10.690s (self 0.097s) [anon child 0.007s] [child 10.592s]
    ...
    dlog("pass 1 #wall_set = %d", (function(t)local c=0;for _ in pairs(t)do c=c+1;end;return c;end)(wall_set))          --     576  0.007s--     576
    ...
    for _, block in ipairs(all_blocks) do                                                                               --     239
        local bk = ("%d,%d,%d"):format(block.map_pos.x, block.map_pos.y, block.map_pos.z)                               --    1214
        if not S.processed_blocks[bk] then                                                                              --     357
            S.processed_blocks[bk] = true                                                                               --     225
            local f, w, u = scan_block(block, existing, wall_set, in_site, road_set)                                    --     897
            tf = tf + f; tw = tw + w; tu = tu + u                                                                       --     652
        end
    end
    runtime = (runtime + getTimestamp()) / getTimestampDivisor()
